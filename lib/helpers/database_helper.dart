...
class DatabaseHelper {
  ...
  Future<Database> _initDB(String filePath) async {
    ...
    // 🔥 Versi database dinaikkan ke 7 untuk penambahan kolom bukti_nota
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    // 🔥 Migrasi versi 7: Menambahkan kolom bukti_nota ke tabel yang sudah ada
    if (oldVersion < 7) {
      try { await db.execute("ALTER TABLE pengeluaran ADD COLUMN bukti_nota TEXT"); } catch (_) {}
    }
  }
  ...
  Future<int> deleteTask(int id) async {
    ...
    if (idDivisi != 0) await syncDivisiStatus(idDivisi); 
    return rows;
  }
  ...
  // 🔥 Fungsi Update dengan penerimaan parameter buktiNota
  Future<bool> insertPengeluaranWithValidasi({
    required int divisiId, required String tanggal, required String namaBarang, required int jumlah, required int nominal, String? buktiNota,
  }) async {
    final db = await instance.database;
    return await db.transaction<bool>((txn) async {
      ...
      await txn.insert('pengeluaran', {
        'divisi_id': divisiId, 'tanggal': tanggal, 'nama_barang': namaBarang, 'jumlah': jumlah, 'nominal': nominal, 'bukti_nota': buktiNota,
      });
      return true; 
    });
  }
  ...
  Future<bool> updatePengeluaranWithValidasi({
    required int id, required int divisiId, required String tanggal, required String namaBarang, required int jumlah, required int nominal, String? buktiNota,
  }) async {
    final db = await instance.database;
    return await db.transaction<bool>((txn) async {
      final divisiRows = await txn.query('divisi', where: 'id = ?', whereArgs: [divisiId], limit: 1);
      ...
      await txn.update('pengeluaran', {
        'tanggal': tanggal, 'nama_barang': namaBarang, 'jumlah': jumlah, 'nominal': nominal, 'bukti_nota': buktiNota,
